-- Upgrade the original empty chat schema while preserving existing user profiles.
-- Counts were verified before this migration: chats, invites, members, and messages were all empty.

drop function if exists public.consume_chat_invite(uuid, text);
